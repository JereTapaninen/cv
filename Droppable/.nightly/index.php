<?php
	...
	require_once('./bin/firstphase.php');
...
<html>
	...
	<body>
		...
		<?php
			include_once('./bin/mainhandler.php');
		?>
		...
		<footer>
			<div>
				...
				<p id="smalltext">Droppable&trade; for Web - Version 0.1601 (Nightly Alpha)</p>
			</div>
		</footer>
		...
		<div id="hiddenstuff" style="display: none; visibility: hidden">
			...
			<form action="./" method="POST">
				<input id="downloadpageurl" name="downloadpageurl" type="text" value="<?php if ($toDownload && $goAhead) echo $downloadpurl ?>" />
		...
		<script type="text/javascript">
			...
			if ($("#downloadpageurl").attr("value").indexOf("download.php") > -1 || $("#downloadpageurl").attr('value').indexOf(".html") > -1 || $("#downloadpageurl").attr('value').indexOf(".mp3") > -1 || $("#downloadpageurl").attr('value').indexOf("NULL") > -1)
